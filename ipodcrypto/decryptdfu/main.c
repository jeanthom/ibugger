...
void decrypt2(uint32_t* data, uint32_t size)
{
    uint32_t ptr, i, buf0[4], buf1[4];
    PWRCONEXT &= ~0x400;
    AESTYPE = 2;
    AESUNKREG0 = 1;
    AESUNKREG0 = 0;
    ...
    for (ptr = 0; ptr < size; ptr += 4)
    {
        for (i = 0; i < 4; i++) buf0[i] = data[ptr + i];
        AESOUTADDR = (uint32_t)data + (ptr << 2);
        AESINADDR = (uint32_t)data + (ptr << 2);
        AESAUXADDR = (uint32_t)data + (ptr << 2);
        clean_dcache();
        ...
        AESGO = 3;
        invalidate_dcache();
        for (i = 0; i < 4; i++) data[ptr + i] ^= buf1[i];
        for (i = 0; i < 4; i++) buf1[i] = buf0[i];
    }
    AESCONTROL = 0;
    PWRCONEXT |= 0x400;
}
...
void main(void)
{
    ...
    decrypt(image, 0x200);
    uint32_t size = image[3];
    if (size > 0x1f800) size = 0x1f800;
    decrypt(&image[0x200], size >> 2);
}
